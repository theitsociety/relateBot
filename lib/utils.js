...
class Utils extends BaseHelper {
  ...
  getEmailsByRoles(role) {
    ...
    allGroupMembers.forEach(member => {
      ...
      if (_.get(member, "Communication Preferences", []).includes("Do you want to be informed about event schedules?")) {
        optedIn.push(member["Email Address"]);
      } else {
        optedOut.push(member["Email Address"])
      } 
    });
  ...
  prepareEmailBulkUploadFile(role, emails, withHeader) {
    const output = [];
    if (withHeader) {
      output.push("Group Email [Required],Member Email,Member Name,Member Role,Member Type");
    }
    emails.forEach(email => {
      output.push(`${role}@itsociety.org,${email},Member,MEMBER,USER`);
    });
    return output.join("\n");
  }
  ...
  _addFields (fields, source, inline = false) {
    for (const key in source) {
      const value = !_.isArray(source[key]) ? (source[key] || "\u200B") : source[key].join(source[key][0].length > 25 ? '\n' : ', ');
      fields.push({
