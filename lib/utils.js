...
class Utils extends BaseHelper {
  ...
  async correlateDiscordWithPartner (guild) {

    if (guild && !this.config.startUpCorrelatation) {
      return
    }
    ...
    if (!guild) {
      guild = this.client.guilds.cache.get(this.config.guildId);
  ...
  async checActivityInMentorshipChannels(guild) { 
    ...
    try {
      ...
      await mentorshipCategory.children.cache.forEach(async mentorshipChannel => {
        ...
        if (lastMessage.createdTimestamp && Date.now() - lastMessage.createdTimestamp > this.config.mentorship.remindAfter) {
          ...
          for (let [guildMember] of mentorshipChannel.members) {
            ...
            // Exclude admins and mentor-relation group member in the follow-up messagd
            if (!member.user.bot && !member.permissionsIn(mentorshipChannel).has("Administrator") && !member.roles.cache.some(role => _.includes(this.config.mentorship.coordinatorRoles, role.id))){
              users += `<@${member.id}>, `;
          ...
          \nThis is ${this.config.platform.name} mentorship service. We are reaching out to check on the status of the mentorship service. Can you please provide us with an update on the following:\n * The overall progress\n * Any challenges or issues that have arisen in the service.\n * Any upcoming milestones
      ...
      this.logger(`Checking activity in mentorship channels failed but will retry: ${e}`)
        ...
        setTimeout((guild) => this.checActivityInMentorshipChannels(guild), this.config.syncPeriodActivityInMentorshipChannels, guild);
    }
  }

  async synMemberData(guild) {
    ...
    try {
      ...
      for ( const data of userData) {
        ...
        if (data.id) {
          ...
          if (Date.now() - new Date(data["Last edited time"]) > this.config.registrations.remindAfter) {
            if (data.Status == Enum.Status.INVITATION_SENT) {
              await this.createInvite(data["Email Address"], data, "renewInvite");
        ...
        if (data["Onboarding Status"] == Enum.Status.ASSIGNED && 
          ...
          if (communityBuilder) {
            this.emailPartnerClient.sendEmailWithTemplate('onboardingReminderEmail', { 
        ...
        } else if (data["Onboarding Status"] == Enum.Status.COMMUNICATION_PENDING && 
            data["Onboarding Communication Reminded"] != true &&
            Date.now() - new Date(data["Last edited time"]) > this.config.onboarding.remindAfter) {
          await this.dataPartnerClient.update({ 
            ..._.pick(data, ["partnerId", "Email Address"]), 
            "Onboarding Communication Reminded": true
          });
          const communityBuilder = await this.dataPartnerClient.fetchById(data["Assigned Community Builder"]);
          if (communityBuilder) {
            this.emailPartnerClient.sendEmailWithTemplate('onboardingSecondReminderEmail', { 
              email: communityBuilder["Email Address"], 
              member: data["Full Name"],
              communityBuilder: communityBuilder["Full Name"]
            }).then( messageId => {
              this.logger(`Onboarding second reminder email sent to **${communityBuilder["Full Name"]} (${communityBuilder["Email Address"]})**: ${messageId}`, { logChannel: this.config.channels.communityBuildersChannel});
            }).catch(e => {
              this.logger(`Unable to send onboarding second reminder e-mail to **${communityBuilder["Email Address"]}**: ${e}`);
            })
          }
        }
      }
      ...
      if (_.get(this.dataPartnerClient, `partnerConfig.request.referralDatabase`)) {
      ...
      setTimeout((guild) => this.synMemberData(guild), this.config.syncPeriod, guild);
    }
  }

  async collectRoleSelections() {
    if (_.isEmpty(this.config.channels.roleSelectionChannel)) {
      return;
    }
  ...
  async manageEmailGroupsMembership(member, newRoles = [] , oldRoles = []) {
    // this.logger(`Old groups: ${oldRoles}, New groups: ${newRoles}, shouldSendEmail: ${this.shouldSendEmail(member)}`);
    try {
      if (!this.shouldSendEmail(member) || !this.emailPartnerClient.partnerConfig.email.enableEmailGroups) {
        return;
      }
  ...
  async inviteReferral(email, messageId, renewInvite = false, referral) {
    ...
    if (!referral) {
      ...
      result.error = `${email} already registered to ${this.config.platform.name}`;           
    } else {
      if (!renewInvite) {
        this.logger(`**${referral["Referrer Name"]}** invited **${email}** to ${this.config.platform.name}`);
      }
      let Status;
      await this.emailPartnerClient.sendEmailWithTemplate('referralEmail',{ 
        email, 
  ...
  async finalizeReferral(email) {
    ...
    await this.emailPartnerClient.sendEmailWithTemplate('referrerAckEmail', { 
  ...
  async invitationPrehook(member) {
    await this.manageEmailGroupsMembership(member, [ ...(member.groups || []), this.config.roles.all]);
  }

  async landingPosthook(member, memberOnPartner) {
    const guild = this.client.guilds.cache.get(this.config.guildId);
    (memberOnPartner.groups || []).forEach(groupName => {
      const role = guild.roles.cache.find(role => role.name === groupName);
      if (!role) {
        return;
      }
      member.roles.add(role)
        .then(mem => this.logger(`**${groupName}** role assigned to **${this.getNickname(mem)}**`))
        .catch(error => this.logger(`Error while assigning group or setting nickname: ${error}`));  
    });

    member.setNickname(_.trim(memberOnPartner['Full Name']), 'Updated nickname with user registration')
      .then(mem => this.logger(`User nickname updated to **${this.getNickname(mem)}**`))
      .catch(error => this.logger(`Error while assigning group or setting nickname: ${error}`)); 
  }

  async createInvite(email, member, inviteType) {
    ...
    if (!member) {
      ...
      if (!inviteType) {
        if (member.Status == Enum.Status.REGISTERED) {
          await this.invitationPrehook(member);
        } else if (member.Status == Enum.Status.LEFT_DISCORD) {
          inviteType = "joinBackInvite";
        } 
      }
      ...
      this.emailPartnerClient.sendEmailWithTemplate('welcomeEmail',{ email, invite: result.newInvite.url, type: inviteType }).then( messageId => {
        Status = inviteType ? Enum.Status.INVITATION_RESENT : Enum.Status.INVITATION_SENT;
        this.logger(`${inviteType ? "Reinvitation" : "Invitation"} e-mail sent to **${email}**: ${messageId}`);
      }).catch(e => {
  ...
  async sendDonationReceipt(content) {
    const inputArr = content.split('\n').map(e => {
      const flds = e.split(':');
      flds.shift();
      return flds.join(':').trim();
    });
    if (inputArr.size<8 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inputArr[2])) {
    ...
    const inputObj = {
      ...
      email: inputArr[2],
      amount: inputArr[3],
      date: new Date(inputArr[4].replaceAll('-','.')).toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}),
      invoiceNumber: inputArr[6],
      receiptLink: inputArr[7]
    }
    return await this.emailPartnerClient.sendEmailWithTemplate('donationReceipt', inputObj).then( messageId => {
      this.logger(`Donation receipt e-mail sent to **${inputObj.email}**: ${messageId}`);
      inputObj.messageId = messageId;
      return inputObj;
    }).catch(e => {
      this.logger(`Unable to send invitation e-mail sent to **${inputObj.email}**: ${e}`);
      throw e;
    });  
  }
  ...
  async relateRecentlyConsumedInviteCode(member) {
    try {
      ...
      if (this.recentlyConsumedInviteCode && this.invites.get(this.recentlyConsumedInviteCode)) {
        ...
        userInfo["Status"]= _.isEmpty(memberOnPartner.groups) ? Enum.Status.LANDED_ON_DISCORD : Enum.Status.ROLE_SELECTED;
        ...
        await this.landingPosthook(member, memberOnPartner);
        if (!this.shouldSendEmail(userInfo)) {
          this.logger(`Landing email is skipped for  **${nickname}**`);
        } else {
          await this.emailPartnerClient.sendEmailWithTemplate('landingEmail',{ email: userInfo['Email Address'] }).then( messageId => {
  ...
  async purgeMemberData(member) {
    ...
    this.logger(`**${this.getNickname(member)}** left ${this.config.platform.name} Discord Server`);
    if (amember) {
  ...
  prepareEmailBulkUploadFile(role, results, withHeader) {
    ...
    results.forEach(el => {
      output.push(`${role}@${this.emailPartnerClient.partnerConfig.email.domain},${el["Email Address"]},${el["Full Name"]},MEMBER,USER`);
    });
    return output.join("\n");
  }
  ...
  generateUserInfoEmbed (userInfo) {
    ...
    this._addFields(fields, _.pick(userInfo, ["Full Name", 
      "Job Title", 
      ...
      "Would you like to join our career mentoring program?",
      ...
      "embeds": [
        {
          ...
          fields,
          "thumbnail": {
            "url": `${this.config.platform.icon}`,
            "proxy_url": `${this.config.platform.url}`,
            "height": 0,
            "width": 0
          },
          "author": {
            "name": `${this.config.platform.name}`,
            "url": `${this.config.platform.url}`
          }
        }
      ]
    }
  ...
  _addFields (fields, source, inline = false) {
    for (const key in source) {
      ...
      fields.push({
        "name": key === "\u200B" ? key : _.startsWith(key, '/') ? key : _.startCase(key),
        value: (value || "\u200B"),
  ...
  async createProfile(userInfo) {
    ...
    userInfo["Communication Preferences"] = [
      ...
      `Do you want us to share with you additional opportunities that The ${this.config.platform.name} members can benefit fr`
    ];
    userInfo["Onboarding Status"] = Enum.Status.PENDING;
    await this.dataPartnerClient.insert({ ...userInfo });
    return this.generateEmbed("New Registration", _.pick(userInfo, [..._.values(_.get(this.config, `partnerConfig[${this.config.dataPartner}].optionMappings`, {}))], "Communication Preferences"), false);
  }
  ...
  async createOnboardingChannel(member, communityBuilder, roles=[]) {
    const channelName = this.purify(member["Full Name"],'-');
    const channel = await this.createPrivateChannel(channelName, roles.concat([member.id, communityBuilder.id]), `Onboarding ${member["Full Name"]}`, this.config.channels.onboardingCategory);
    await channel.send(`<@${member.id}> welcome to ${this.config.platform.name} Discord Server. \n\n<@${communityBuilder.id}> will assist in your onboarding process. Please use this private channel to reach out for any questions you might have.`);  
    return channel;
  }

  async createMentorChannel(category, mentor, mentee, topic, roles=[]) {
    const channelName = category + "-" + this.purify(mentee["Full Name"], '-');
    const channel = await this.createPrivateChannel(channelName, roles.concat([mentor.id, mentee.id]), topic || `Mentoring ${mentee["Full Name"]}`, this.config.channels.mentorshipCategory);
    await channel.send(`**Welcome onboard <@${mentee.id}>,**
      ...
      \na) ${this.config.platform.name} uses DISCORD as its communication tool.
      \nb) Please do not use any other platform for mentor/mentee communication.
      \nc) There are multiple channels available for use such as ”${this.config.mentorship.voiceChannels.join("”, ”")}“ under VOICE CHANNELS which can be used for meetings.
    ...
    await channel.send(`** **
      ...
      \nb) Meeting action items should be posted in the channel and due dates should be included, alternatively, you can use the page created for your meeting notes. The address of the page can be seen in the Discord “channel topic”.\n
    ...
    if (!member) {
      ...
      if (!communityBuilder|| !communityBuilder.partnerId) {
        ...
        if (member.id && communityBuilder.id) {
          const channel = await this.createOnboardingChannel(member, communityBuilder, this.config.mentorship.coordinatorRoles);
        ...
        await this.emailPartnerClient.sendEmailWithTemplate('onboardingAssignmentEmail', { 
  ...
  async assignMentor(options) {
    ...
    if (!mentor || !mentor["Email Address"]) {
      ...
      result.error = `${options.category} is not one of supported categories: ${_.keys(this.config.mentorship.domains).join(", ")}`;
    }else {
      const assignment = await this.dataPartnerClient.upsert({ 
        Name: mentee["Full Name"],
        Domain: this.config.mentorship.domains[options.category],
        Mentor: mentor.partnerId,
        Mentee: mentee.partnerId,
        Status: Enum.Status.ONGOING,
        "Email Address": mentee["Email Address"],
        "Mentor Assignment Date": new Date().toISOString().split('T')[0],
        ...(options.page && { URL: options.page })
      }, _.get(this.dataPartnerClient, `partnerConfig.request.mentorshipDatabase`));
      result.success += `Assignment entry is created.\n`; 
      if (!options.page) {
        options.page = _.get(assignment, "url");
      }
      result.success += `Please visit the page and grant access to mentor/mentee.\n${options.page}\n`;

      const channel = await this.createMentorChannel(options.category, mentor, mentee, options.page, this.config.mentorship.coordinatorRoles);
      ...
      await this.emailPartnerClient.sendEmailWithTemplate('mentorAssignmentEmail', { 
  ...
  async getUserInfo(member) {
    ...
    if (userInfo["Email Address"]) {
      const data = this.dataPartnerClient.partnerConfig.request.skillDatabase ?
        await this.dataPartnerClient.fetchWithSkills(userInfo["Email Address"]) : 
        await this.dataPartnerClient.fetch(userInfo["Email Address"]);
      if (data) {
