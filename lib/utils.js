...
class Utils extends BaseHelper {
  constructor(client, config) {
    ...
    const clientFactory = new ClientFactory(config, config.dataPartner);
    this.dataPartnerClient = clientFactory.getClient();
  }
  ...
  purify (input) {
    return this.allReplace(input.replace(/\s+/g, '').toLowerCase(), { "ü": "u", "ğ": "g", "ı": "i", "ş":"s", "ç": "c", "ö": "o"});
  }

  async correlateDiscordvsNotion () {

    this.logger(`Correlation analysis has started at ${new Date}`, { consoleOnly: true});

    const guild = this.client.guilds.cache.find(guild => guild.name === 'IT Society');
    ...
    members.forEach(async (member) => {
      ...
      if (match) {
        ...
        // console.log(`No match: ${nickname}`)
      }
    });
  
    this.logger(`Correlation analysis has ended at ${new Date}`, { consoleOnly: true});
  ...
  async synMemberData() {
    try {
      ...
      this.logger(`Member data sync has failed but will retry: ${e}`)
    } finally {
  ...
  async collectRoleSelections() {
    const roleChannel = this.client.channels.cache.find(channel => channel.name === "🔴select-role");
  ...
  async createInvite(email) {
    ...
    if (!member) {
      ...
      await this.dataPartnerClient.update({ ..._.pick(member, ["partnerId", "Email Address"]), invite: result.newInvite.code});
      this.logger(`Invite created for **${email}** ${result.newInvite}`);
    } 
    return result;
  }
