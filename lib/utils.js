...
class Utils extends BaseHelper {
  ...
  async assignMentor(options) {
    ...
    if (!mentor || !mentor["Email Address"]) {
      result.error = `Registration not found for **${utils.getNickname(options.mentor)}**`;
    } else if (!mentee || !mentee["Email Address"]) {
      result.error = `Registration not found for **${utils.getNickname(options.mentee)}**`;
    } else {
      const channel = await this.createMentorChannel(options.category, mentor, mentee, options.page);
      result.success = `Created private <#${channel.id}> channel for communication.`;
