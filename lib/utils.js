    ...
    try {
        ...
        if (data.id) {
          ...
          if (Date.now() - new Date(data["Last edited time"]) > this.config.registrations.remindAfter) {
            if (data.Status == Enum.Status.INVITATION_SENT) {
              ...
              await this.dataPartnerClient.update({ ..._.pick(data, ["partnerId", "Email Address"]), Status: Enum.Status.RETIRED});
              this.logger(`**${data["Full Name"]}** did not land on Discord even after reinvitation. Retiring registration...`);
            }
          } else {
      ...
      for ( const referral of reinvitedReferrals) {
        if (Date.now() - new Date(referral["Last edited time"]) > this.config.referrals.remindAfter) {
          await this.dataPartnerClient.update({ ..._.pick(referral, ["partnerId", "Email Address"]), Status: Enum.Status.RETIRED});
          this.logger(`**${referral["Referral Name"]}** did not register even after reinvitation. Retiring invite...`);
          ...
          await message.react(this.config.referrals.retiredEmoji);
        }
      }   
