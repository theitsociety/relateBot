...
class Utils extends BaseHelper {
  ...
  async createPrivateChannel (name, users = [], reason, parent) {
    const options = {
      name,
  ...
  async assignMember(options) {
    ...
    if (!member) {
      ...
      if (!communityBuilder|| !communityBuilder.partnerId) {
        result.error = `Registration not found for **${utils.getNickname(options.communityBuilder)}**`;
      } else {
        ...
        result.success = `**${member["Full Name"]}** assigned to Community Builder **${communityBuilder["Full Name"]}** successfully.`;

        if (member.id && communityBuilder.id) {
          const channelName = await this.createOnboardingChannel(member, communityBuilder);
          result.success += `\nCreated **${channelName}** channel for communication`;
        }
  
        // TODO: Send email to both community builder and new member
