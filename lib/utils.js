...
class Utils extends BaseHelper {
  ...
  async manageEmailGroupsMembership(member, newRoles = [] , oldRoles = []) {
    try {
      ...
      if (!_.isEmpty(groupsToBeAdded)) {
        this.logger(`Adding **${member['Full Name']}** to **${groupsToBeAdded.join(", ")}** email group(s)`);  
        await this.emailPartnerClient.addMemberToGroups(groupsToBeAdded, member['Email Address'], member['Full Name']);
      }
      ...
      if (!_.isEmpty(groupsToBeRemoved)) {
        this.logger(`Removing **${member['Full Name']}** from **${groupsToBeRemoved.join(", ")}** email group(s)`);  
  ...
  async relateRecentlyConsumedInviteCode(member) {
    try {
      ...
      if (this.recentlyConsumedInviteCode && this.invites.get(this.recentlyConsumedInviteCode)) {
        ...
        await this.manageEmailGroupsMembership(userInfo, "all");
      } else {
  ...
  async purgeMemberData(member) {
    ...
    if (amember) {
      ...
      await this.manageEmailGroupsMembership(amember, undefined, amember.groups);
  ...
  getEmailsByRoles(role) {
    if (role != "all" && !Object.values(this.config.roles).includes(role)) {
      return "Not a correlated role";
    }
    const allGroupMembers = Array.from(this.members.values())
      .filter(member => role == "all" || _.get(member, "groups", []).includes(role));
  ...
  async getUserInfo(member) {
    ...
    if (userInfo["Email Address"]) {
      const data = await this.dataPartnerClient.fetch(userInfo["Email Address"]);  
      if (data) {
  ...
  async redeemProfile(userInfo, updates, email) {
    ...
    if (_.get(data, 'id') && _.get(data, 'id') != userInfo.id) {
      ...
      else if (!updates["Full Name"]){ 
        result.error = `Name is required to create profile`;
      } else {
        _.extend(userInfo, updates);
