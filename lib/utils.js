...
class Utils extends BaseHelper {
    ...
    try {
      ...
      for ( const data of userData) {
        ...
        if (data["Onboarding Status"] == Enum.Status.ASSIGNED && 
          ...
          const communityBuilder = await this.dataPartnerClient.fetchById(data["Assigned Community Builder"]);
          if (communityBuilder) {
            this.emailPartnerClient.sendOnboardingSecondReminderEmail({ 
              email: communityBuilder["Email Address"], 
              member: data["Full Name"],
              communityBuilder: communityBuilder["Full Name"]
            }).then( messageId => {
  ...
  async createInvite(email, member, inviteType) {
    ...
    if (!member) {
      ...
      this.emailPartnerClient.sendInvite({ email, invite: result.newInvite.url }, inviteType).then( messageId => {
  ...
  async sendDonationReceipt(content) {
    ...
    return await this.emailPartnerClient.sendDonationReceipt(inputObj).then( messageId => {
