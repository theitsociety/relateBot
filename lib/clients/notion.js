...
class NotionClient extends BaseClient {
  ...
  async fetch(contains, start_cursor, database = this.partnerConfig.request.registationDatabase) {
      ...
      transformResponse: [
        (data) => {
          ...
          if (resp.results) {
            ...
            return typeof contains == 'string' ? _.get(resp, 'results.0') : resp;
          } else {
  ...
  _prepareUpdateData(update) {
    ...
    for (const key of ["Communication Preferences", "Groups"]) {
    ...
    for (const key of ["Assigned Community Builder", "Mentor"]) {
      const discordKey = _.get(keymappings, key, key);
      if (update[discordKey]) {
        data.properties[key] = {
          "relation":[
    ...
    for (const key of ["Onboarding Communication Reminded"]) {
      if (_.has(update, key)) {
        data.properties[key] = {
          checkbox: update[key]
        }
